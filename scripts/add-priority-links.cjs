...
for (const file of files) {
  ...
  // Replace an earlier generated block so changes to the link map are applied
  // consistently on subsequent builds instead of being skipped forever.
  html = html.replace(/\s*<section data-seo-priority-links="true"[\s\S]*?<\/section>\s*/gi, '\n');

  const group = GROUPS.find(g => g.match.test(url));
  const block = group
    ? makeBlock(group)
    : makeBlock({ title: 'Popular Pixaroid Tools', paths: PRIORITY });

  // Most modern pages have </main>; the standalone International pages use
  // a body/container layout. Insert before </main> when available, otherwise
  // before </body>, keeping the SEO links inside the document body.
