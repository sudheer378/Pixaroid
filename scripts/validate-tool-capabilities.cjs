...
const runnerTypes = new Set(['compress','compress-target','convert','convert-multi','convert-pdf','resize','resize-social','social-canvas','ai','utility','filter','editor']);
const specialTypes = new Set(['pdf','legacy','special']);
const errors = [];
const seen = new Set();
...
for (const t of entries) {
  ...
  if (!t.workerFile || t.workerFile.includes('bulk.worker.js')) errors.push(`Invalid/removed worker mapping: ${t.slug}`);
