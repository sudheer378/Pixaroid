...
function walk(dir) {
  for (const name of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (name.isFile() && name.name === 'index.html') rows.push(p);
  }
}
...
for (const file of rows) {
  ...
  const body = attr(html, /<body[^>]*>([\s\S]*?)<\/body>/i).replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ');
  const words = (body.match(/\b[\p{L}\p{N}][\p{L}\p{N}'’-]*\b/gu) || []).length;

  const row = { file: rel, title, description, robots, canonical, expectedCanonical: expected, h1, schema, words };
  rows.push;
...
for (const [title, files] of seenTitles) if (files.length > 1) issues.push({ severity: 'MEDIUM', type: 'duplicate-title', title, files });
for (const [canonical, files] of seenCanonicals) if (files.length > 1) issues.push({ severity: 'HIGH', type: 'duplicate-canonical', canonical, files });
