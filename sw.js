...
function _isShell(path) {
  return path === '/' || path.endsWith('.css') ||
    (path.startsWith('/js/') && path.endsWith('.js') && !path.includes('/chunks/')) ||
    (path.startsWith('/workers/') && path.endsWith('.js')) || path === '/manifest.json';
}
function _isAsset(path) {
  return path.startsWith('/assets/') || path.endsWith('.svg') || path.endsWith('.png') ||
...
self.addEventListener('sync', event => {
  if (event.tag === 'pxn-offline-queue') event.waitUntil(Promise.resolve());
});
